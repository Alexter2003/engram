...
- `POST /projects/migrate` — Rescue explicitly selected historical records whose project ownership is `NULL`. Body: `{target_project, confirmed:true, observation_ids?:[], session_ids?:[], prompt_ids?:[]}`
  - A configured `ENGRAM_HTTP_TOKEN`, matching `Authorization: Bearer <token>`, `target_project`, `confirmed:true`, and at least one concrete record ID are required.
  - Owned records are never reassigned. The response reports rescued, conflicting, and skipped records plus whether canonical mutations were journaled locally.
  - A local journal is not a cloud acknowledgement; autosync reports subsequent reconciliation state.
