...
- `POST /projects/rescue-ownership` — Rescue explicitly selected historical records whose project ownership is `NULL`. `POST /projects/migrate` is a deprecated compatibility alias routed to the same handler. The JSON body is limited to 8 KiB: `{target_project, confirmed:true, observation_ids?:[], session_ids?:[], prompt_ids?:[]}`.
  ...
  - `200` returns `{status, target_project, rescued_observations, rescued_sessions, rescued_prompts, conflicting_records, skipped_records, journaled_local, reconciliation_status}`. Owned records are never reassigned.
  - Ownership moves atomically: in the same transaction, the parent session of every selected observation or prompt is rescued to `target_project` first. A record whose parent session already belongs to a different project is reported under `conflicting_records` and left untouched, so a record is never split from its session. `rescued_sessions` therefore counts dependent parent sessions as well as explicitly requested ones. A blank project is treated exactly like `NULL` — neither identifies an owner — and no sync mutation is ever journaled for a blank-owned record.
